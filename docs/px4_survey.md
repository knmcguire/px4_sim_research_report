...
The most urgent category involves regressions introduced in recent PX4 versions. [@px4-41925] [@px4-41925] exemplifies this problem: parameters `SIM_GZ_HOME_LAT`, `SIM_GZ_HOME_LON`, and `SIM_GZ_HOME_ALT` were removed in v1.15.0, making it impossible to spawn drones at custom positions—a fundamental requirement for multi-vehicle simulations and mission testing. Similarly, [@px4-46594] reports that Hardware-in-the-Loop (HITL) actuators ceased functioning in v1.15.4, breaking a critical testing workflow for hardware validation.

Platform-specific build failures also fall into this critical category. Multiple users reported incompatibilities with macOS 15.0 (Sequoia) and difficulties with Apple Silicon builds. These issues are particularly problematic as they represent complete blockers: users cannot proceed with any simulation work until resolved. [@px4-40832], [@px4-40889], [@px4-43606], [@px4-44661], [@px4-44172], [@px4-45887], and [@px4-46712] collectively indicate systematic problems with build toolchains across different operating systems and versions.

### Core Simulation Stability (Priority 2)

The Extended Kalman Filter (EKF) and positioning subsystems show recurring instability patterns. [@px4-40657] describes extreme position drift and jumping estimates—sometimes up to 50 meters—during basic hovering in a Docker-based Gazebo Harmonic setup. The user noted that "even when hovering, the copter experiences extreme drifts, which is very unusual for a simulation without wind." Similar reports appear in [@px4-45701], [@px4-46410], and [@px4-45340], suggesting systematic problems with position estimation when using custom worlds, external visual odometry, or non-standard sensor configurations. Additional EKF-related issues include [@px4-40696], [@px4-41693], [@px4-38605], [@px4-42050], [@px4-42291], [@px4-42720], [@px4-43607], and [@px4-41928].

Flight behavior issues frequently prevent basic operations. Multiple users report drones that arm successfully but fail to take off ([@px4-43349], [@px4-44266], [@px4-44442]), with some drones unable to climb above 0.1 meters despite proper command inputs. [@px4-39412] describes heavy drones that "climb unresponsively after takeoff," indicating problems with thrust-to-weight modeling or controller tuning in simulation. [@px4-44460] reports takeoff failures after controller modifications, while [@px4-45612] shows VTOL-specific takeoff problems. Additional flight issues include [@px4-45275] and [@px4-45842].

Timing and performance issues also affect simulation usability. [@px4-47188] highlights a fundamental limitation: custom simulators running slower than real-time cannot function with PX4 SITL due to lockstep timing assumptions. The user found their 15Hz sensor update rate caused EKF failures and failsafe triggers, as "PX4 expects ~50 Hz real-time updates." This constraint severely limits integration with high-fidelity physics simulators or resource-constrained environments. [@px4-43347] reports ROS subscribers dramatically slowing simulation, while [@px4-44546] describes environment-specific crashes. [@px4-47475] requests simulation speed improvements.
...
The ROS2 bridge exhibits systematic reliability problems affecting sensor data flow. [@px4-44447] and [@px4-44810] report failures in the ros_gz_bridge preventing camera image topics from reaching RViz2, despite the topics being visible in Gazebo's native tools. [@px4-47421] describes scan topics that never publish messages, breaking SLAM toolbox integration. [@px4-43428] reports similar sensor data failures, while [@px4-43476] specifically affects lidar. [@px4-45529] describes problems launching ROS with PX4 SITL, and [@px4-45852] reports general topic publication failures. [@px4-46126] highlights visual odometry topic issues, while [@px4-46979] shows the disconnect between Gazebo and ROS2 topic visibility. These communication failures force users to implement workarounds or abandon ROS2 integration entirely.

Offboard control through ROS2 shows inconsistent behavior. [@px4-45073] reports that direct actuator control fails to respond, while [@px4-42010] describes VTOL-specific offboard control challenges. [@px4-44630] notes frame convention problems with `MAV_FRAME_BODY_NED`, causing unexpected offsets. [@px4-42506] and [@px4-46447] report general offboard mode difficulties. These issues suggest incomplete or inconsistent implementation of the MAVLink-to-ROS2 control path.

### Custom Models and Airframes (Priority 5)

Users attempting to create custom vehicle models encounter inadequate documentation and unpredictable behavior. [@px4-43417] describes an octocopter that "only takes off at 0.8m and does not reach the desired altitude" after modifying mass and inertia properties, even with increased motor velocity limits. This suggests disconnects between physical model parameters and PX4's internal vehicle assumptions.

[@px4-46241] highlights the difficulty of determining correct motor constants for Gazebo models. [@px4-46132] identifies a potential bug in the GZ bridge ESC interface, where the relationship between PWM values and motor commands lacks clear physical meaning. A developer comment acknowledges: "The main problem is that there is no good model for an ESC...What is slightly not elegant is probably how this translates to rotor velocity or thrust." This architectural uncertainty makes accurate custom vehicle modeling unnecessarily difficult. [@px4-34747] requests clearer motor parameter definitions, while [@px4-40607], [@px4-40575], [@px4-43858], [@px4-46547], [@px4-6700], and [@px4-47541] represent various custom model creation challenges.

Specialized vehicle types show particular gaps. Multiple issues ([@px4-46487], [@px4-46582], [@px4-46561]) report VTOL tilt-rotors diving dangerously during transitions. [@px4-46838] notes reversible motors cannot actually reverse in SITL. [@px4-28805] describes coaxial octocopter simulation problems, while [@px4-32781] requests airship simulation support. Additional specialized vehicle issues include [@px4-18807], [@px4-42031], [@px4-42077], [@px4-42324], [@px4-43654], [@px4-5207], [@px4-46457], [@px4-46469], [@px4-46758], [@px4-47231], and [@px4-47263]. These suggest the simulation framework optimizes for standard quadcopters at the expense of other vehicle types.

### Sensor Simulation (Priority 6)

Camera and vision sensors show inconsistent functionality. [@px4-42909] and [@px4-43155] report video streams that are "unavailable" despite proper configuration, breaking computer vision workflows. [@px4-45077] describes hardware encoding errors in Gazebo camera plugins, while [@px4-45711] and [@px4-45952] highlight camera integration difficulties. [@px4-43035] addresses point cloud visualization challenges. The inability to reliably stream simulated camera data undermines visual navigation, object detection, and SLAM research.
...
Users seek integration with non-Gazebo simulators but face insufficient interfaces. [@px4-42508] requests Unity integration, while [@px4-45970] reports inability to connect MAVSDK with PX4 SITL in AirSim/Unreal Engine. Multiple issues ([@px4-42303], [@px4-44244], [@px4-25459], [@px4-43561], [@px4-46425]) describe problems with MATLAB/Simulink integration, an important platform for control system development in aerospace. [@px4-44512] requests integration with Simcenter Amesim.

[@px4-47152] asks about "custom simulator in SITL," highlighting that the current architecture assumes Gazebo. The response to issue #30 confirms that simulators running slower than real-time are fundamentally incompatible with PX4's lockstep scheduler. This inflexibility prevents integration with high-fidelity simulators common in aerospace (e.g., FlightGear, X-Plane) or game engines popular for reinforcement learning research.

### Docker and Deployment (Priority 8)

Containerized simulation shows deployment challenges. [@px4-46059] reports generic failures when running PX4 SITL in containers, while [@px4-32048] describes connection problems with dockerized simulations. [@px4-45983] specifically notes video feeds fail to forward from Docker containers—a critical limitation for headless server deployments or cloud-based simulation infrastructure.

Graphics rendering issues compound Docker problems. [@px4-44619] reports Gazebo entities that spawn successfully but remain invisible, while [@px4-39733] describes Gazebo failing to load entirely. [@px4-43070] shows general Gazebo startup failures, and [@px4-45702] addresses VM-specific challenges. These problems suggest OpenGL/EGL configuration difficulties in containerized environments, limiting the simulation's utility in CI/CD pipelines or distributed testing frameworks.

### World and Environment (Priority 9)

Custom world integration lacks robustness. [@px4-43167], [@px4-46990], and [@px4-47171] report QGroundControl failing to connect when using custom world files, despite the simulation appearing to run correctly. [@px4-47409] indicates that custom worlds cause magnetometer initialization problems in v1.16, requiring manual parameter adjustments that aren't documented. [@px4-43738] addresses version compatibility with custom worlds, while [@px4-46242] reports arming failures in custom environments.

Environmental effects show limited support. [@px4-42846] asks how to simulate altitude effects on lift—fundamental for high-altitude operations—but receives no clear answer. [@px4-45696] reports the documented windy environment command (`make px4_sitl gz_x500_windy`) fails to produce wind effects. These limitations prevent realistic testing of environmental robustness.

### Parameter and Configuration (Priority 10)

Parameter access and modification during simulation shows gaps. [@px4-47359] reports `PARAM_REQUEST_LIST` MAVLink commands failing to return complete parameter sets, breaking automated configuration tools. [@px4-47399] asks about changing MAVLink ports for multiple ground stations but finds the process undocumented. [@px4-40908] addresses parameter access in offboard mode, while [@px4-41678] requests PWM configuration guidance.

Simulation-specific configurations lack flexibility. [@px4-42119] requests the ability to disable failsafes during testing—a reasonable development need—but finds no supported mechanism. [@px4-42015] asks whether drone instances can start in "clean state" regardless of previous flights, important for reproducible experiments. [@px4-42586] describes time synchronization problems between components, while [@px4-41998] questions network configuration options. [@px4-42754] requests GPS disabling for VIO testing, [@px4-43097] addresses external odometry configuration, and [@px4-47082] seeks to disable Gazebo's odometry publisher. [@px4-46035] highlights sensor frame orientation challenges, [@px4-43846] requests arbitrary state initialization for ML, and [@px4-44400] reports external vision aiding issues. [@px4-43777] and [@px4-46164] address data bridging challenges, [@px4-46632] reports bridge startup failures, and [@px4-46694] questions rotor velocity bridging. Additional configuration issues include [@px4-46698], [@px4-46856], [@px4-44060], [@px4-46000], and [@px4-47434]. These issues indicate the simulation environment doesn't adequately support the iterative development workflows users need.
