...
## Key changes compared to Pilot survey
...
Forum analysis (in the Appendix) revealed that version compatibility as a recurring crisis, with breaking changes between PX4 releases removing critical features like custom spawn positions. Multiple users expressed frustration that “things worked in one configuration, but does not in another.” So the final survey adds explicit questions aversion compatibility issues and importance as well.
